...
# Provisions the GitHub repo for playwright-trace-llm.
# Adapted from the go-project template's provision-repo.sh:
#   - required status checks match this repo's CI jobs (lint, build, test)
#   - the "Human review gate" environment/deployment rule is intentionally omitted
#   - the Copilot/gh-aw janitor authentication step is omitted (no janitor here)
# Applies repo settings, a branch ruleset, and a tag ruleset that templates don't carry over.
